...
EXPLANATION OF PROCESS AND CODE

A 10 bit up counter, counting from 0 to 2^10-1 in binary, was plugged into the
FPGA to represent a 10 bit ADC. The bits were generated by Digilent's Analog Discovery box.
This box offers many features including digital signal generation, which was used for this 
test. I have used this device many times and I did a sanity check by plugging the generated
up counter waveform back into its digital analyzer to verify the output, so I have faith in
the accuracy and performance of this device.

The 10 bit data was read into the FPGA and stored in a FIFO. Then, via a third party
API, Front Panel (FP), developed by Opal Kelly (OK), I was able to request the data from
the FIFO, through the USB link, and onto the computer. I then wrote the received data into
a text file for process and analysis.

The reason the sampled code is seperated at 255 (a total of 256 samples) is
due to the OK, FP USB link transmission limitations. 
